...
## 快速启动：旅行 LoRA 微调版 API

 `api_server.py` 会使用 **DiffSynth `WanVideoPipeline`**，启动后会加载：

- 基座模型：`Wan-AI/Wan2.2-TI2V-5B`
- 微调 LoRA：`satoshinji/wan22-ti2v-5b-travel-lora`
- 默认分辨率：`832*480`
- 默认帧数：`49`
- 默认采样步数：`30`
- 默认 LoRA 强度：`0.8`

> 微调代码、数据清单、微调记录、部署报告在 `finetune/travel_lora/`。

### 1. 克隆项目

```bash
git clone https://github.com/satoshinji2992/wan2.2.git
cd wan2.2
```

### 2. 准备 Python 环境

建议使用 Python 3.10。下面以 conda 为例：
...
如果 `pip install diffsynth` 在你的环境里不可用，改用源码安装：
...
```bash
mkdir -p finetune/travel_lora/weights
...
```text
finetune/travel_lora/weights/travel_lora_step380.safetensors
```
...
curl -X POST http://127.0.0.1:8000/generate \
  -F "image=@palace_museum.png" \
  -F "prompt=A cinematic travel documentary view of the Forbidden City, ancient red palace walls and golden roofs, calm daylight, smooth forward camera movement." \
  -F "size=832*480" \
  -F "sample_steps=30" \
  -F "frame_num=49" \
  -F "seed=1" \
...
```text
finetune/travel_lora/README.md
finetune/travel_lora/LOCAL_USAGE.md
finetune/travel_lora/dataset/metadata.csv
finetune/travel_lora/docs/FINETUNE_RECORD.md
finetune/travel_lora/docs/DEPLOYMENT_REPORT.md
finetune/travel_lora/scripts/collect_archive_travel_videos.py
finetune/travel_lora/scripts/train_wan22_ti2v_lora_local.sh
finetune/travel_lora/scripts/infer_wan22_ti2v_lora.py
```

### 9. 常见问题

- `lora_loaded=false`：检查 `--lora_path` 指向的文件是否存在。
- 第一次启动很慢：`--load_on_start` 会在启动时加载基座模型和 LoRA，这是正常的。
- 显存不足：先用 `832*480`、`frame_num=49`、`sample_steps=30`；不要一开始就用 720P 或 121 帧。
- 多个请求同时进来：当前 API 使用单 GPU 队列锁，同一时间只跑一个生成任务，避免显存互相抢占。
- 如果 DiffSynth 找不到模型：确认 `./Wan2.2-TI2V-5B` 下有 `diffusion_pytorch_model-00001-of-00003.safetensors`、`models_t5_umt5-xxl-enc-bf16.pth`、`Wan2.2_VAE.pth`。
