...
> 系统开发阶段使用 Flask 和 Vite 方便调试；部署阶段使用 Nginx + 多个 Gunicorn 后端实例。Nginx 开启多 worker 处理静态资源和请求转发，并通过 least_conn 在多个 Gunicorn 实例之间分发 `/api` 和 `/uploads` 请求；每个 Gunicorn 实例内部还可以启动多个 worker/线程处理 Flask 请求。这样可以避免单进程 Flask 开发服务器成为瓶颈，也为后续扩展到更多后端实例或多台服务器保留结构。
